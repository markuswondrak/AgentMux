...
2. Create a detailed design handoff in `04_design/design.md` for any new UI views/components. For each created artifact (for example `.css` or `.html`), include an `Integration Instructions` section that tells the coder which classes to use and where the CSS import must be added.
