...
When developing AgentMux itself, you need to use the installed version of the tool while working on the source code. The `agentmux-dev` wrapper script solves this by using the pipx-installed Python interpreter and removing the current directory from Python's import path.

### Prerequisites

You must have agentmux installed via pipx (not editable install):
...
When you run `agentmux` from within the repository, Python finds the local `agentmux/` package before the installed version. This means code changes immediately affect the running tool, which is problematic when using AgentMux to develop AgentMux.

### Using the wrapper

```bash
# Use the wrapper instead of the regular command
./agentmux-dev "Add rate limiting to the API"
./agentmux-dev init
./agentmux-dev resume
```

### Installing the wrapper (optional)

For convenience, you can symlink the wrapper to your PATH:
...
This ensures Python imports the installed version from the pipx venv's site-packages instead of the local source.
