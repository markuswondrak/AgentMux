...
class ProjectPromptExtensionsRequirementsTests(unittest.TestCase):
    def _write_project_prompt(
        self, project_dir: Path, subdir: str, name: str, content: str
    ) -> None:
    ...
    def _write_shared_fragment(
        self, prompts_dir: Path, name: str, content: str
    ) -> None:
    ...
    def test_shared_fragment_placeholders_are_rendered_with_template_format_map(
        self,
    ) -> None:
        with tempfile.TemporaryDirectory() as td:
            tmp_path = Path(td)
    ...
    def test_builders_render_bracketed_placeholders(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            ...
            files = create_feature_files(
                project_dir, feature_dir, "bracket placeholders", "session"
            )
    ...
    def test_shared_fragments_can_provide_bracketed_placeholders(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            ...
            project_dir.mkdir()
            files = create_feature_files(
                project_dir, feature_dir, "shared bracket placeholders", "session"
            )
    ...
    def test_legacy_format_placeholders_are_left_literal(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            ...
            project_dir.mkdir()
            files = create_feature_files(
                project_dir, feature_dir, "placeholder compatibility", "session"
            )
    ...
    def test_project_prompt_curly_braces_remain_literal_with_bracketed_placeholders(
        self,
    ) -> None:
        with tempfile.TemporaryDirectory() as td:
            tmp_path = Path(td)
            ...
            project_dir.mkdir()
            files = create_feature_files(
                project_dir, feature_dir, "project prompt safety", "session"
            )
    ...
    def test_shared_fragments_preserve_project_instruction_curly_brace_safety(
        self,
    ) -> None:
        with tempfile.TemporaryDirectory() as td:
            tmp_path = Path(td)
    ...
    def test_builders_inject_project_prompt_extensions_before_constraints(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            ...
            project_dir.mkdir()
            files = create_feature_files(
                project_dir, feature_dir, "project-specific prompts", "session"
            )
            ...
            cases: list[tuple[str, str, str, object]] = [
                (
                    ...
                    "architect",
                    "EXT-ARCHITECT",
                    lambda runtime: build_architect_prompt(runtime),
                ),
                (
                    "agents",
                    "product-manager",
                    "EXT-PM",
                    lambda runtime: build_product_manager_prompt(runtime),
                ),
                (
                    "agents",
                    "reviewer",
                    "EXT-REVIEWER",
                    lambda runtime: build_reviewer_prompt(runtime),
                ),
                (
                    "agents",
                    "designer",
                    "EXT-DESIGNER",
                    lambda runtime: build_designer_prompt(runtime),
                ),
                (
                    "agents",
                    "coder",
                ...
                (
                    ...
                    "review",
                    "EXT-REVIEW-COMMAND",
                    lambda runtime: build_reviewer_prompt(runtime, True),
                ),
                (
                    "commands",
                    "fix",
                    "EXT-FIX",
                    lambda runtime: build_fix_prompt(runtime),
                ),
                (
                    "commands",
                    "confirmation",
    ...
    def test_planning_and_coder_prompts_keep_docs_work_in_main_implementation_scope(
        self,
    ) -> None:
        with tempfile.TemporaryDirectory() as td:
            tmp_path = Path(td)
            ...
            project_dir.mkdir()
            files = create_feature_files(
                project_dir, feature_dir, "docs scope", "session"
            )
            self._write_change_inputs(feature_dir)
            self._write_coder_inputs(feature_dir, plan_name="plan_1.md")
    ...
    def test_coder_subplan_prompt_includes_completed_research_references(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            ...
            project_dir.mkdir()
            files = create_feature_files(
                project_dir, feature_dir, "research handoff", "session"
            )
            self._write_coder_inputs(feature_dir, plan_name="plan_1.md")

            self._create_research_topic(
                feature_dir, "web-openai-models", done=True, include_detail=False
            )
            self._create_research_topic(
                feature_dir, "code-auth-module", done=True, include_detail=True
            )
            self._create_research_topic(
                feature_dir, "code-incomplete-topic", done=False, include_detail=True
            )
    ...
    def test_coder_subplan_prompt_includes_completed_research_references(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            ...
            self._create_research_topic(
                feature_dir, "web-routing", done=True, include_detail=True
            )
            self._create_research_topic(
                feature_dir, "code-db-indexes", done=True, include_detail=True
            )
    ...
    def test_coder_prompts_omit_research_handoff_when_no_completed_topics(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            ...
            self.assertNotIn(
                "Research handoff (read before new exploration):", subplan_prompt
            )
            self.assertNotIn(
                "03_research/code-incomplete-topic/summary.md", subplan_prompt
            )
    ...
    def test_cross_prompt_preference_guidance_and_proposal_paths_remain_consistent(
        self,
    ) -> None:
        with tempfile.TemporaryDirectory() as td:
            tmp_path = Path(td)
            ...
            for prompt_name, prompt in rendered_prompts.items():
                with self.subTest(prompt=prompt_name):
                    self.assertIn(
                        "approve, edit, or dismiss each candidate", prompt.lower()
                    )
                    self.assertIn(
