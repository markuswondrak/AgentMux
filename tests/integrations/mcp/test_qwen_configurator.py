...
from agentmux.integrations.mcp import McpServerSpec
from agentmux.integrations.mcp.configurators import QwenConfigurator


class QwenConfiguratorTests(unittest.TestCase):
    """Tests for QwenConfigurator class."""
    ...
    def setUp(self) -> None:
        self.configurator = QwenConfigurator()

    def test_provider_is_qwen(self) -> None:
        """provider attribute equals 'qwen'."""
        self.assertEqual(self.configurator.provider, "qwen")

    def test_config_path_returns_home_qwen_settings(self) -> None:
        """config_path(project_dir) → ~/.qwen/settings.json."""
        with tempfile.TemporaryDirectory() as td:
            ...
            with patch(
                ...
                expected = home_dir / ".qwen" / "settings.json"
    ...
    def test_prompt_message_contains_path_and_provider(self) -> None:
        """prompt_message includes qwen provider label and config path."""
        with tempfile.TemporaryDirectory() as td:
            home_dir = Path(td)
            project_dir = home_dir / "project"
            project_dir.mkdir()
            ...
            self.assertIn(str(home_dir / ".qwen" / "settings.json"), msg)
