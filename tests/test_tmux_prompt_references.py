...
class TmuxPromptReferencesTests(unittest.TestCase):
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            def fake_run_command(args, cwd=None, check=True):
                ...
                if args[:2] == ["tmux", "split-window"]:
                    ...
                    return CompletedProcess(
                        args=args, returncode=0, stdout=f"{pane}\n", stderr=""
                    )
            ...
            with (
                ...
                patch(
                    "agentmux.runtime.tmux_control.accept_trust_prompt",
                    return_value=None,
                ),
            ):
            ...
            border_format_cmd = next(
                ...
                for cmd in commands
                if cmd[:5]
                == ["tmux", "set-option", "-t", "session-x", "pane-border-format"]
            )
            ...
            self.assertIn("#{?#{@pane_label},", border_format_cmd[5])
            self.assertNotIn("·", border_format_cmd[5])
            split_index = commands.index(
                next(cmd for cmd in commands if cmd[:2] == ["tmux", "split-window"])
            )
    ...
    def test_enforce_monitor_min_width_resizes_when_monitor_is_too_narrow(self) -> None:
        ...
        def fake_run_command(args, cwd=None, check=True):
            ...
            commands.append(list(args))
            if (
                args[:4] == ["tmux", "display-message", "-p", "-t"]
                and args[-1] == "#{pane_width}"
            ):
                return CompletedProcess(
                    args=args, returncode=0, stdout="18\n", stderr=""
                )
            return CompletedProcess(args=args, returncode=0, stdout="", stderr="")
    ...
    def test_enforce_monitor_min_width_resizes_when_monitor_is_too_wide(self) -> None:
        ...
        def fake_run_command(args, cwd=None, check=True):
            ...
            if (
                ...
                return CompletedProcess(
                    args=args, returncode=0, stdout="52\n", stderr=""
                )
            return CompletedProcess(args=args, returncode=0, stdout="", stderr="")
    ...
    def test_content_zone_show_parallel_rebalances_visible_content_panes(self) -> None:
        ...
        self.assertEqual(
            ...
            [
                cmd
                for cmd in commands
                if cmd[:2] in (["tmux", "join-pane"], ["tmux", "select-layout"])
            ],
        )
        width_mock.assert_called_once_with("session-x")
    ...
    def test_content_zone_hide_rebalances_remaining_visible_panes(self) -> None:
        ...
        self.assertEqual(
            ...
            [
                cmd
                for cmd in commands
                if cmd[:2] in (["tmux", "break-pane"], ["tmux", "select-layout"])
            ],
        )
        width_mock.assert_called_once_with("session-x")
    ...
    def test_create_agent_pane_applies_display_label_without_overwriting_role(
        self,
    ) -> None:
        agents = {
            "coder": AgentConfig(
        ...
        def fake_run_command(args, cwd=None, check=True):
            ...
            if args[:2] == ["tmux", "split-window"]:
                return CompletedProcess(
                    args=args, returncode=0, stdout="%9\n", stderr=""
                )
            return CompletedProcess(args=args, returncode=0, stdout="", stderr="")

        with (
            patch(
                "agentmux.runtime.tmux_control._find_any_hidden_pane", return_value="%2"
            ),
            patch("agentmux.runtime.tmux_control._pane_in_window", return_value=False),
            patch(
        ...
        self.assertIn(
            ["tmux", "set-option", "-p", "-t", "%9", "@role", "coder"], commands
        )
        self.assertIn(
            ["tmux", "set-option", "-p", "-t", "%9", "@pane_label", "API wiring"],
            commands,
        )
